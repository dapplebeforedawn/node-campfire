...
Room.prototype.stopListen = function() {
  console.log("no longer listening on room.");
...
Room.prototype.listen = function(callback) {
  ...
  (function listen() {
    console.log(this._stopListen);
    if (this._stopListen){return;}
    campfire.http.request(options, function(response) {
      response.setEncoding("utf8");
      response.on("end", listen);
      response.on("error", function(e){
        console.log("campfire http error: ", e);
      });
